...
// BoxliteNetworkConnect accepts an HTTP CONNECT request and relays the
// hijacked byte stream to a guest port. CONNECT is the raw-stream transport
// used by SDK/CLI clients; browser preview and WebSocket terminal stay on
// their existing handlers.
func BoxliteNetworkConnect(logger *slog.Logger) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		r, err := runner.GetInstance(nil)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		...
		if err != nil {
			logger.WarnContext(ctx.Request.Context(), "guest CONNECT dial failed", "box", boxId, "port", port64, "error", err)
		...
		if !ok {
			_ = guestConn.Close()
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": "CONNECT is not supported by the HTTP server"})
			return
		}
		...
		relayBidirectional(ctx.Request.Context(), clientConn, guestConn, logger)
	}
}

func relayBidirectional(parentCtx context.Context, clientConn, guestConn net.Conn, logger *slog.Logger) {
	...
	go func() {
		_, err := io.Copy(guestConn, clientConn)
		errCh <- err
	}()
	go func() {
		_, err := io.Copy(clientConn, guestConn)
		errCh <- err
	}()
	...
	case err := <-errCh:
		if err != nil && logger != nil {
			logger.DebugContext(ctx, "CONNECT tunnel closed", "error", err)
		}
	case <-ctx.Done():
	}
}
