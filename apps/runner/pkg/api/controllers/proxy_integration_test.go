...
func TestIntegrationGuestPortTunnelRelaysHTTPViaRealVM(t *testing.T) {
	...
	var stderr integrationTestBuffer
...
type integrationTestBuffer struct {
	mu sync.Mutex
	b  strings.Builder
}

func (b *integrationTestBuffer) Write(p []byte) (int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.b.Write(p)
}

func (b *integrationTestBuffer) String() string {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.b.String()
}
