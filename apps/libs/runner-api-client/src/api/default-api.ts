...
export const DefaultApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async runnerInfo(options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<RunnerInfoResponseDTO>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.runnerInfo(options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['DefaultApi.runnerInfo']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
    }
};
...
export class DefaultApi extends BaseAPI {
    ...
    public runnerInfo(options?: RawAxiosRequestConfig) {
        return DefaultApiFp(this.configuration).runnerInfo(options).then((request) => request(this.axios, this.basePath));
    }
}
