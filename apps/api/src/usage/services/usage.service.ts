...
export class UsageService implements TrackableJobExecutions, OnApplicationShutdown {
  ...
  private async createUsagePeriod(event: BoxStateUpdatedEvent, diskOnly = false) {
    ...
    usagePeriod.kind = diskOnly ? 'stopped' : 'running'
    if (!diskOnly) {
    ...
    usagePeriod.boxClass = event.box.class ?? BoxClass.SMALL
    usagePeriod.regionType = await this.getRegionType(event.box.region)
    usagePeriod.actualCpuSeconds = null
    usagePeriod.actualRssAvgBytes = null
    usagePeriod.actualRssPeakBytes = null
    usagePeriod.sampleCount = null
  ...
  @WithInstrumentation()
  async closeAndReopenUsagePeriods(now: Date = new Date()) {
    ...
    const usagePeriods = await this.usagePeriodRepository.find({
      where: {
        endAt: IsNull(),
        startAt: LessThan(new Date(now.getTime() - 1000 * 60 * 60 * 24)),
