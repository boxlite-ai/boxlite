...
import { MeteringMode } from '../../usage/metering/metering-policy'
...
export class RuntimeLeaseService {
  ...
  private expiresAt(observedAt: Date): Date {
    const leaseSeconds = this.config.getOrThrow('billing.runtimeLeaseSeconds')
    if (!Number.isSafeInteger(leaseSeconds) || leaseSeconds <= 0) {
      throw new Error('billing.runtimeLeaseSeconds must be a positive integer')
    }
