...
export function createBoxToCreateBox(dto: RestCreateBoxDto, target?: string): CreateBoxDto {
  ...
  // Persist launch configuration only when explicitly supplied to preserve existing create behavior.
  ...
  if (hasLaunchConfig) {
    createDto.launchConfig = {
      ...
      tty: dto.tty,
      detach: dto.detach,
      // detach=false means the CLI expects local-like streaming output and the main process exit code.
      foreground: dto.detach === false,
      // Record cleanup intent; the lifecycle flow performs deletion after the foreground process exits.
