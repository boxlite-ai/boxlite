...
import { apiScopesFor, RestApiScope } from './api-scope'
...
export class BoxliteMeController {
  ...
  async getMe(@AuthContext() ctx: AuthCtx): Promise<PrincipalDto> {
    ...
    const scopes = apiScopesFor(await this.withMembership(ctx, pathPrefix))
  ...
  /**
   * The caller as the resource guard would see it, membership included.
   *
   ...
   */
  private async withMembership(ctx: AuthCtx, organizationId: string | null): Promise<OrganizationAuthContext> {
    if (ctx.apiKey || !organizationId) {
      return ctx as OrganizationAuthContext
    }

    const organizationUser = await this.organizationUserService.findOne(organizationId, ctx.userId)
    return { ...ctx, organizationId, organizationUser } as OrganizationAuthContext
  }
