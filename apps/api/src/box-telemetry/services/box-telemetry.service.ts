...
@Injectable()
export class BoxTelemetryService {
  private readonly logger = new Logger(BoxTelemetryService.name)
  ...
  async getTraces(boxId: string, from: string, to: string, page: number, limit: number): Promise<PaginatedTracesDto> {
    const serviceName = this.getServiceName(boxId)
    const offset = (page - 1) * limit
  ...
  async getTraceSpans(boxId: string, traceId: string): Promise<TraceSpanDto[]> {
    const serviceName = this.getServiceName(boxId)
  ...
  async getMetrics(boxId: string, from: string, to: string, metricNames?: string[]): Promise<MetricsResponseDto> {
    const serviceName = this.getServiceName(boxId)
