...
func run() int {
	...
	if proxy.ClickStackGatewayEnabled() {
		...
		configuredLogger, shutdownLogger, err := initLogger(ctx, logger, proxyConfig)
		if err != nil {
		...
		if proxyConfig.OtelTracingEnabled && proxyConfig.OtelEndpoint != "" {
			logger.Info("OpenTelemetry tracing is enabled")

			tp, err := telemetry.InitTracer(ctx, telemetry.Config{
				Endpoint:       proxyConfig.OtelEndpoint,
				Headers:        proxyConfig.GetOtelHeaders(),
		...
		start = func(ctx context.Context) error {
			return proxy.StartProxy(ctx, proxyConfig)
		}
	}
