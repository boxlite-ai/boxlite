...
Budget for compute, managed state, storage, networking, and operations; the architecture's dollar markers include services with free allowances.

# Infrastructure cost catalog

[Infrastructure index](../README.md) · [Architecture graphs](architecture.md) · [Configuration](configuration.md)
...
For AWS, start from the same workload inventory and map it through the
[AWS provider](../mdeploy/stack/providers/aws/): ECS, EC2/EBS, RDS, ElastiCache, S3, ECR, ALB/NLB,
CloudFront, NAT instances/public IPv4, Secrets Manager/SSM, CloudWatch and optional SES.

Source inventory: [GCP providers](../mdeploy/stack/providers/gcp/) and [GCP bootstrap](../bootstrap/gcp.ts).
