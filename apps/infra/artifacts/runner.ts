...
export function runnerArtifactsBucketName({ app, stage, accountId }: any) {
...
function requireBuildLocation(source: any, environment: any) {
 ...
 * }}
 */
export function resolveRunnerArtifact(source: any, environment = process.env) {
...
export function artifactFetchCommand(artifact: any, url: any, destination: any, region: any) {
...
async function runAwsCli(awsCliPath: any, args: any, description: any, { environment, timeoutMs, signal }: any) {
  try {
...
async function verifyBuildArtifact(source: any, { awsCliPath, environment, timeoutMs, signal }: any) {
  const { bucket, tarballName, key } = requireBuildLocation(source, environment)
...
export async function verifyRunnerArtifact(source: any, { environment = process.env, timeoutMs = 15_000, ...cli }: any = {}) {
  if (source.kind !== 'release' && source.kind !== 'build') {
    throw new Error(`unknown Runner artifact source '${source.kind}'`)
  }
  if (source.kind === 'release') {
