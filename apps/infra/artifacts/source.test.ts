...
const resolve = (component: any, environment: any) => resolveArtifactSource(component, environment, workspace)
...
test('a component ref wins over the global one, the way its source key does', () => {
  ...
  const runner = (environment: any) => resolve('runner', { ...build, ...environment })
...
test('every ref is checked, not only the first component that has one', () => {
  ...
  assert.throws(
    ...
    (error: any) => error.message.startsWith(`RUNNER_ARTIFACT_REF is ${REF}`),
  )
  ...
  assert.throws(
    ...
    (error: any) => error.message.startsWith(`RUNNER_ARTIFACT_REF is ${other}`),
  )
})
