 ...
 *
 * Usage: npm run login -- [--only aws,github] [--force]
...
function isCliInstalled(command: any) {
...
async function confirm(question: any) {
 ...
 */
async function offerInstall(provider: any) {
  const recipe = installCommand(provider.install)
  if (!recipe) throw new Error(`no install recipe is configured for ${provider.label}`)
...
function isAuthenticated(provider: any) {
  try {
 ...
 */
function runLogin(provider: any) {
...
async function main() {
  ...
  const providers = selectProviders(only ? only.split(',').map((key: any) => key.trim()).filter(Boolean) : undefined)
  ...
  for (const provider of providers) {
    ...
    if (!cliInstalled) {
      ...
      const missing = decideMissingCliAction({
        ...
        managerAvailable: recipe !== null && isCliInstalled(recipe.command),
      })
