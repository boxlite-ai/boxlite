...
const stripShell = (text) => text.replace(/(^|[\s;|&(<>)])#.*$/gm, '$1')
...
const stripScript = (text) => text.replace(/^[ \t]*\/\*[\s\S]*?\*\//gm, '').replace(/(^|[^:])\/\/.*$/gm, '$1')
...
const STRIPPERS = {
  ...
  scriptEmittingShell: (text) => stripShell(stripScript(text)),
}

export function liveText(kind, text) {
