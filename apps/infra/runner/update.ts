 ...
 *
 * The binary comes from whichever source artifacts/source.ts selects: the published
...
import { readWorkspaceVersion } from '../deployment/environment.js'
import { artifactFetchCommand, resolveRunnerArtifact } from '../artifacts/runner.js'
...
// deployment/sst.ts and keeps the remote payload dependency-free.
...
const sleepSeconds = (seconds: any) => spawnSync('sleep', [String(seconds)])
...
function upgradeOne(instanceId: any, upgrade: any) {
...
export function runRunnerUpdateCli() {
  try {
    main()
  } catch (err: any) {
