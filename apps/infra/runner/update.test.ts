...
const assertShellLine = (run: any, pattern: any) => assert.match(liveText('shell', run), pattern)
...
test('the checksum manifest must name the tarball literally, not by pattern', () => {
  ...
  assert.ok(awkLine, 'the payload must contain its checksum extraction')
...
test('sendCommand hands SSM a base64 payload with no shell metacharacters', () => {
  ...
  const flag = (name: any) => seen[seen.indexOf(name) + 1]
...
test('sendCommand retries an unregistered agent but not a permission failure', () => {
  const slept: any[] = []
  const sleep = (seconds: any) => slept.push(seconds)
...
test('runs as a script from a path that needs percent-encoding', (t) => {
  ...
  const copiedInfra = join(root, 'infra')
  for (const domain of ['runner', 'artifacts', 'deployment', 'shared']) {
    cpSync(join(infraDirectory, domain), join(copiedInfra, domain), { recursive: true })
  }
