...
const AUDITED_PROVIDER_DEFAULTS = new Map<string, unknown>([
  ['forceDestroy', false],
...
export function normalizeRunnerProtectedInputs(properties: any) {
  if (!properties || typeof properties !== 'object' || Array.isArray(properties)) {
    throw new Error('invalid Runner inputs')
  }

  const protectedInputs: Record<string, any> = {}
...
export function createRunnerSafetyFingerprint(properties: any) {
...
function readConsistentIdentityTag(properties: any, key: any) {
  const values = []
...
function legacyControlPlaneRunnerName(resourceName: any, nameTag: any) {
...
export function createRunnerIdentityFingerprint(properties: any, resourceName: any, { allowLegacyFallback = false } = {}) {
...
export function hasExactIgnoredProperties(ignoreChanges: any) {
...
export function createRunnerStateBaseline(exportedState: any) {
  ...
  const runnerResources: Record<string, { inputFingerprint: string; identityFingerprint: string }> = {}
...
export function parseRunnerStateBaseline(serializedBaseline: any) {
  let baseline
  try {
  ...
  for (const [name, properties] of Object.entries(baseline.resources) as Array<[string, any]>) {
    if (
