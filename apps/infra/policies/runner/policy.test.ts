...
// eslint-disable-next-line @nx/enforce-module-boundaries -- The policy tests exercise the shared CommonJS Runner model.
import runnerInventory from '../../runner/model/inventory.js'
// eslint-disable-next-line @nx/enforce-module-boundaries -- The policy tests exercise the shared CommonJS Runner state model.
import runnerStateBaseline from '../../runner/model/state-baseline.js'
import runnerPolicyDefinitions from './definitions.js'
import runnerPolicy from './validate.js'
...
const RUNNER_POLICY_ENTRY = resolve(process.cwd(), 'policies/runner/index.ts')
...
function runnerResource(spec: any, overrides = {}) {
  return {
    type: RUNNER_TYPE,
...
function runnerBaseline(inventory: any, includedInventory = inventory) {
  const includedNames = new Set(includedInventory.map((runner: any) => runner.resourceName))
  return {
    version: 3,
    resources: Object.fromEntries(
      inventory
        .filter((runner: any) => includedNames.has(runner.resourceName))
        .map((runner: any) => [
          runner.resourceName,
          {
...
function collectViolations(validate: any) {
  const violations: any[] = []
  validate((message: any) => violations.push(message))
  return violations
}
...
test('requires the serialized Runner state baseline before loading the policy pack', () => {
  ...
  const result = spawnSync(process.execPath, ['--import', 'tsx', RUNNER_POLICY_ENTRY], {
    encoding: 'utf8',
    env: environment,
    maxBuffer: 1024 * 1024,
    timeout: 5_000,
...
test('rejects Runner properties that cannot be resolved during preview', () => {
  const inventory = resolveRunnerInventory({})
  const baseline = runnerBaseline(inventory)
  const resource: any = runnerResource(inventory[0])
...
test('wires mandatory resource and stack validators to policy reports', () => {
  ...
  assert.ok(resourcePolicy?.validateResource)
  assert.ok(stackPolicy?.validateStack)
  assert.deepEqual(
    collectViolations((reportViolation: any) =>
