...
import { CreateApiKeyPermissionsEnum, OrganizationRolePermissionsEnum, type ApiKeyResponse } from '@boxlite-ai/api-client'
...
const SCENARIOS = [
  {
    ...
    description:
      'Run AI-generated or untrusted code in an isolated, disposable Box. Create a key, install the SDK, then execute code safely inside a sandbox.',
  },
] as const
...
export function OnboardingGuideDialog({ open, onOpenChange, onProgressChange, progress }: OnboardingGuideDialogProps) {
  ...
  const handleCreateKey = async () => {
    ...
    try {
      const key = (
        await apiKeyApi.createApiKey({ name: 'sdk-quickstart', permissions: apiKeyPermissions }, selectedOrganization.id)
      ).data
      setCreatedKey(key)
                ...
                <button type="button" onClick={() => setStep(i)} className="flex flex-none items-center gap-[9px]">
                  ...
                  <span
                    className={cn(
                      ...
                      active ? 'font-semibold text-foreground' : isDone ? 'text-foreground' : 'text-muted-foreground',
                    )}
                  >
                    {s.label}
                  </span>
          ...
          {step === 0 && (
            <div className="px-5 py-[18px]" style={{ animation: 'stat-in .25s ease' }}>
              ...
              <div className="flex items-center gap-[10px] border border-border bg-[hsl(var(--code-background))] px-[14px] py-3">
                <span
                  className={cn('flex-1 break-all text-[13px] tracking-[0.5px]', createdKey ? 'text-foreground' : 'text-muted-foreground')}
                >
              ...
              {createdKey && (
                <div className="mt-[11px] flex items-start gap-2 text-[11.5px] leading-relaxed text-muted-foreground">
                  ...
                  <span>
                    Save this as <span className="text-foreground">BOXLITE_API_KEY</span> in your environment
                    (e.g. <code className="text-foreground">export BOXLITE_API_KEY=…</code>) — the SDK reads it at runtime
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                {createdKey ? (
                  <PrimaryBtn onClick={() => complete(0)}>{done[0] ? '✓ Secured · Next' : 'Copied · Next →'}</PrimaryBtn>
                ) : (
          ...
          {step === 1 && (
            <div className="px-5 py-[18px]" style={{ animation: 'stat-in .25s ease' }}>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        on ? 'border-brand font-semibold text-foreground' : 'border-transparent text-muted-foreground',
                      )}
                    >
              ...
              <div className="mt-4 flex justify-end">
                <PrimaryBtn onClick={() => complete(1)}>{done[1] ? '✓ Installed · Next' : 'Installed · Next →'}</PrimaryBtn>
              </div>
            </div>
          )}
            ...
            <div className="px-5 py-[18px]" style={{ animation: 'stat-in .25s ease' }}>
              ...
              <div className="mt-[12px] flex items-start gap-2 text-[11.5px] leading-relaxed text-muted-foreground">
                ...
                <span>
                  ...
                  environment, creates a Box, runs a command inside it, prints the output, then removes the Box.
                  Run it in your terminal with the install command from the previous step.
                </span>
              </div>
