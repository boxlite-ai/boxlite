...
/**
 * `Keys.tsx` narrows the offered catalogue to what the operator may grant, so a
 * member holding none of it arrives here with `availablePermissions: []`. Letting
 * that submit mints a key carrying no permission at all: `CreateApiKeyDto` is
 * `@IsArray()` with no `@ArrayNotEmpty()`, so the API stores it, and the key still
 * reaches every box route because those declare no required permission.
 */
describe('CreateApiKeyDialog with nothing the operator can grant', () => {
  let container: HTMLDivElement
  let root: Root
  ...
  beforeEach(async () => {
    ...
    await act(async () => {
      root.render(<CreateApiKeyDialog availablePermissions={[]} apiUrl="http://localhost" organizationId="org-1" />)
    })
    await act(async () => {
      container.querySelector<HTMLButtonElement>('button[title="Create Key"]')?.click()
    })
  ...
  it('refuses to submit a key that would carry no permission at all', async () => {
    await act(async () => {
      typeIntoNameField('empty-key')
    })

    const submit = document.querySelector<HTMLButtonElement>('button[form="create-api-key-form"]')
