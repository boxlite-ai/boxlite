...
// go through this door so the dashboard and the SDKs speak the same dialect;
// cloud-capability reads (paginated list, getBox, signed URLs, ssh, archive)
// stay on the generated api-client.
...
export function toBoxApiCreateRequest(params?: CreateBoxParams): BoxApiCreateRequest {
  ...
  return {
    ...
    network: p.networkBlockAll ? { mode: 'disabled' } : undefined,
    labels: p.labels,
    public: p.public,
