...
async fn async_main() -> BoxliteResult<()> {
    ...
    // Prepare the persistent SSH identity before serving configuration requests.
    // Failure is non-fatal until SSH is enabled, where the lazy path reports the
    // same `SshStartError::HostKey`.
