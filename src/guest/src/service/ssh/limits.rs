...
/// Directory entries returned per SSH_FXP_READDIR response. SFTP paginates
/// readdir through the open dir handle, and clients cap the packet size they
/// will accept (OpenSSH: 256 KiB) -- answering a large directory in one Name
/// packet overruns that and drops the connection.
pub(crate) const MAX_READDIR_ENTRIES_PER_RESPONSE: usize = 256;

/// Deadline for the SSH version exchange and key-exchange setup. A peer that
/// opens a socket and then stalls must not hold an accept-loop permit
/// indefinitely.
pub(crate) const HANDSHAKE_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(30);

/// Deadline from connection accept to a successful certificate authentication.
/// `max_auth_attempts` bounds how many credentials may be tried; this bounds
/// how long an unauthenticated peer may occupy a connection slot.
pub(crate) const AUTH_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(60);

/// Idle limit for an established connection, applied by russh as
/// `Config::inactivity_timeout`. Any received data resets it, so this closes
/// abandoned sessions rather than interrupting quiet interactive ones.
pub(crate) const CONNECTION_IDLE_TIMEOUT: std::time::Duration =
