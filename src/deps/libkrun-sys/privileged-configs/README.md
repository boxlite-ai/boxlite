...
`make libkrunfw-privileged` does:

1. Copy `vendor/libkrunfw/config-libkrunfw_<arch>` → `vendor/libkrunfw/.config`
2. Append the relevant overlay file (this dir, `overlay-privileged_<arch>`)
   to `.config`
...
5. Stamp the SONAME to a distinct filename
   (`libkrunfw-privileged.so.5`) so it can sit next to the lean one
...
`overlay-privileged_aarch64` is currently a copy of the x86_64 overlay since
