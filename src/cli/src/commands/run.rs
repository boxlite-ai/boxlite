...
impl BoxRunner {
    ...
    fn prepare_command(&self) -> BoxCommand {
        ...
        // `--cmd` (gated on `--privileged`) routes args to the
    ...
    fn validate_flags(&self) -> anyhow::Result<()> {
        ...
        // `--privileged` so the non-docker flow stays byte-for-byte
        // identical — there's no path where `--cmd` alone is meaningful.
        if !self.args.management.cmd.is_empty() && !self.args.management.privileged {
            anyhow::bail!("--cmd requires --privileged");
        }
