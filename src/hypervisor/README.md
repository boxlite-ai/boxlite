...
and arm64. The `Vm`, `Vcpu` and `VcpuHandle` traits, with `VcpuExit`,
...
VM operations. The traits are exported from the crate root; each backend's
concrete type, `HvfVm` or `KvmVm`, will get its own export path.
