...
impl PrebuiltRuntime {
    ...
    /// List runtime files, directories, and symlinks from the release artifact.
    fn scan_file_names(&self) -> io::Result<Vec<String>> {
        let mut entries = Vec::new();
        Self::scan_paths(&self.runtime_dir, &self.runtime_dir, &mut entries)?;
        entries.sort();
        Ok(entries)
    }

    fn scan_paths(root: &Path, dir: &Path, entries: &mut Vec<String>) -> io::Result<()> {
        for entry in fs::read_dir(dir)? {
            let entry = entry?;
            let path = entry.path();
            let name = path
                .strip_prefix(root)
                .expect("runtime entry must be below runtime root")
                .to_string_lossy()
            ...
            entries.push(name);
            if metadata.is_dir() {
                Self::scan_paths(root, &path, entries)?;
            }
        }
        Ok(())
    }
...
impl EmbeddedManifest {
    ...
    /// Scan `runtime_dir` recursively for regular files and directories.
    fn scan_entries(&self) -> Vec<(String, Option<PathBuf>, u32)> {
        let mut entries = Vec::new();
        if self.runtime_dir.exists() {
            Self::scan_dir(&self.runtime_dir, &self.runtime_dir, &mut entries);
        }
        entries.sort_by(|a, b| a.0.cmp(&b.0));
        entries
    }

    fn scan_dir(root: &Path, dir: &Path, entries: &mut Vec<(String, Option<PathBuf>, u32)>) {
        for entry in fs::read_dir(dir)
            .unwrap_or_else(|e| panic!("Failed to read runtime dir {}: {e}", dir.display()))
        {
            ...
            if meta.is_dir() {
                entries.push((name, None, Self::file_mode(&path)));
                Self::scan_dir(root, &path, entries);
            } else if meta.is_file() {
                entries.push((name, Some(path.clone()), Self::file_mode(&path)));
            }
        }
    }
    ...
    fn emit_manifest(manifest_path: &Path, entries: &[(String, Option<PathBuf>, u32)]) {
    ...
    fn write_manifest_rs(manifest_path: &Path, entries: &[(String, Option<PathBuf>, u32)]) {
        if entries.is_empty() {
            fs::write(
                manifest_path,
                "// Auto-generated by build.rs — no embedded entries\n\
                 pub const MANIFEST: &[(&str, u32, Option<&[u8]>)] = &[];\n",
            )
        ...
        code.push_str("pub const MANIFEST: &[(&str, u32, Option<&[u8]>)] = &[\n");
            ...
            match path {
                Some(path) => {
                    ...
                    code.push_str(&format!(
                        "    ({name:?}, 0o{mode:o}, Some(include_bytes!({abs:?}))),\n"
                    ));
                    println!("cargo:rerun-if-changed={}", path.display());
                }
                None => code.push_str(&format!("    ({name:?}, 0o{mode:o}, None),\n")),
            }
        }
        code.push_str("];\n");
    ...
    /// Hash manifest names, kinds, modes, and bytes for cache invalidation.
    fn emit_content_hash(entries: &[(String, Option<PathBuf>, u32)]) {
    ...
    fn log_summary(entries: &[(String, Option<PathBuf>, u32)]) {
        let total_size: u64 = entries
            .iter()
            .filter_map(|(_, path, _)| path.as_ref())
            .map(|path| {
                fs::metadata(path)
            ...
            .sum();
        let file_count = entries.iter().filter(|(_, path, _)| path.is_some()).count();
        let dir_count = entries.len() - file_count;
        println!(
            "cargo:warning=Embedded runtime: {} files, {} directories, {:.1} MB total",
            file_count,
            dir_count,
    ...
    /// finds the pre-built shim and guest rootfs, copies them to the runtime dir,
    /// and generates an `include_bytes!` manifest for the complete tree.
    ///
    ...
    fn generate(&self, mode: &DepsMode, cargo: &CargoBuildContext) {
        ...
        self.copy_prebuilt_guest_rootfs(workspace_root, &profile);
    ...
    fn copy_prebuilt_guest_rootfs(&self, workspace_root: &Path, profile: &str) {
        let destination = self.runtime_dir.join("guest-rootfs");
        let _ = fs::remove_file(self.runtime_dir.join("boxlite-guest"));

        let Some(source) = Self::find_prebuilt_guest_rootfs(workspace_root, profile) else {
            let _ = fs::remove_dir_all(&destination);
            println!("cargo:warning=guest-rootfs not found, skipping embed");
            return;
        };

        let _ = fs::remove_dir_all(&destination);
        Self::copy_runtime_tree(&source, &destination).unwrap_or_else(|error| {
            panic!(
                "Failed to copy {} -> {}: {}",
                source.display(),
                destination.display(),
        ...
        println!("cargo:warning=Embedded guest rootfs: {}", source.display());
    }

    fn copy_runtime_tree(source: &Path, destination: &Path) -> io::Result<()> {
        ...
        for entry in fs::read_dir(source)? {
            ...
            if metadata.is_dir() {
                Self::copy_runtime_tree(&source_path, &destination_path)?;
                ...
                return Err(io::Error::other(format!(
                    "unsupported guest rootfs entry: {}",
                    source_path.display()
                )));
            }
        }
    ...
    /// Find the pre-built guest rootfs for the current architecture.
    fn find_prebuilt_guest_rootfs(workspace_root: &Path, profile: &str) -> Option<PathBuf> {
        let target_dir = workspace_root.join("target");
        ...
        for arch in Self::preferred_arches() {
            let path = target_dir
                ...
                .join(profile)
                .join("guest-rootfs");
            if path.join("rootfs/boxlite/bin/boxlite-guest").is_file() {
                return Some(path);
            }
        }
