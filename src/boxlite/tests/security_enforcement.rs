...
#[tokio::test(flavor = "multi_thread")]
async fn custom_capabilities_apply_to_init_and_non_tty_exec() {
    ...
    let init_caps = exec_stdout(
        &bx,
        BoxCommand::new("sh").args(["-c", "grep '^CapEff:' /proc/1/status"]),
    )
    .await;
    assert_capability_change(&init_caps, CAP_SYS_ADMIN, CAP_NET_RAW, "PID 1");

    let exec_caps = exec_stdout(
        &bx,
        BoxCommand::new("sh")
    ...
    .await;
    assert_capability_change(&exec_caps, CAP_SYS_ADMIN, CAP_NET_RAW, "non-TTY exec");

    bx.stop().await.expect("stop box");
    let _ = runtime.shutdown(Some(common::TEST_SHUTDOWN_TIMEOUT)).await;
}

fn assert_capability_change(status: &str, added: u64, dropped: u64, process: &str) {
    let cap_eff = status
        .trim()
        .strip_prefix("CapEff:\t")
        .and_then(|hex| u64::from_str_radix(hex.trim(), 16).ok())
        .expect("CapEff should contain a hexadecimal capability mask");

    assert_ne!(
        cap_eff & added,
        0,
        "CAP_SYS_ADMIN should be effective for {process}, CapEff=0x{cap_eff:x}"
    );
    assert_eq!(
        cap_eff & dropped,
        0,
        "CAP_NET_RAW should not be effective for {process}, CapEff=0x{cap_eff:x}"
    );
}
