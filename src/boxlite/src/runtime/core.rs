...
impl BoxliteRuntime {
    ...
    pub async fn create(
        ...
        // Mounts are judged here as *requested* — one origin, or none for an
        // anonymous mount — for both backends: the local one resolves them
        // next, the REST one forwards them. FFI callers and struct literals
        // can build shapes the constructors cannot, so this is the boundary
        // that catches them; persisted options are judged elsewhere, by
        // `sanitize_persisted`.
