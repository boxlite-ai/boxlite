...
/// so a managed mount arrives carrying both its reference and the payload
/// path of the machine it came from. The request rule would
...
mod tests {
    ...
    fn trusted_import_keeps_persisted_mounts_for_the_runtime_to_resolve() {
        ...
        let volumes = vec![
            ...
            VolumeSpec::bind_mount("/elsewhere/host/data", "/host-data"),
        ];
        let options = BoxOptions {
