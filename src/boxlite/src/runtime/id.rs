...
//!   Base62, [`VolumeID::is_valid`] admits any server-issued string in the
//!   same URL/path-safe set — with one extra duty: a local volume's id is
//!   also its directory name, so the character set is what keeps it a single
//!   path component.
...
/// a REST path.
///
/// One thing this type deliberately does *not* decide is existence: a valid
/// id is a *claim* until the store binds it to a real directory; see
/// [`BoxID::parse`] for the identity discipline that applies equally here.
///
/// # Example
///
/// ```
...
/// assert_eq!(id.as_str().len(), 12);
/// assert!(VolumeID::is_valid("vol_abc-123")); // server-issued styles
/// assert!(!VolumeID::is_valid("../escape")); // never a directory name
/// assert!(!VolumeID::is_valid(""));
/// ```
...
mod tests {
    ...
    /// so anything that could steer that join elsewhere must not be valid.
    #[test]
    fn volume_ids_that_are_not_a_single_path_component_are_invalid() {
        for bad in [
            "",
            ".",
            "..",
            ...
            assert!(!VolumeID::is_valid(bad), "{bad:?} must not be valid");
        }
        let longest = "a".repeat(VolumeID::MAX_LENGTH);
        assert!(VolumeID::is_valid(&longest));
        assert!(!VolumeID::is_valid(&format!("{longest}a")));

        assert!(VolumeID::is_valid("vol_abc-123"));
    }
