...
//! - Volume IDs follow the box rule — [`VolumeIDMint`] mints 12-character
//!   Base62, [`VolumeID::parse`] accepts any server-issued string in the same
//!   URL/path-safe set — with one extra duty: a local volume's id is also its
//!   directory name, so the character set is what keeps it a single path
//!   component.
...
/// Named-volume identifier.
///
/// A volume id is assigned by whoever owns the volume: [`VolumeIDMint`] for
/// volumes created by a local runtime, the server for volumes created over
...
/// a REST path.
///
/// Two things this type deliberately does *not* decide:
/// - `anonymous` parses fine but names the CLI's anonymous-volume subtree, so
///   the store rejects it separately; that reservation is the store's, not the
///   id format's.
/// - Existence. A parsed id is a *claim* until the store binds it to a real
///   directory; see [`BoxID::parse`] for the identity discipline that applies
///   equally here.
///
/// # Example
///
...
/// assert_eq!(id.as_str().len(), 12);
/// assert!(VolumeID::parse("vol_abc-123").is_some()); // server-issued styles
/// assert!(VolumeID::parse("../escape").is_none()); // never a directory name
/// assert!(VolumeID::parse("").is_none());
/// ```
#[derive(Clone, PartialEq, Eq, Hash)]
pub struct VolumeID(String);

impl VolumeID {
    /// Maximum accepted length for any volume id, including server-issued
    /// formats. Matches [`BoxID::MAX_LENGTH`]: both bound an untrusted string
    /// that will be allocated and used as a path component.
    pub const MAX_LENGTH: usize = 128;

    /// Length of the short form used in display (8 chars).
    pub const SHORT_LENGTH: usize = 8;

    /// Parse a volume id from an existing string.
    ///
    /// Returns `None` for empty input, input longer than
    /// [`VolumeID::MAX_LENGTH`], or input outside the URL- and path-safe
    /// character set. Callers must propagate `None` as an error — minting a
    /// replacement id on parse failure would silently address a different
    /// volume than the caller named.
    pub fn parse(s: &str) -> Option<Self> {
        if Self::is_valid(s) {
            Some(Self(s.to_string()))
        } else {
    ...
    /// Check whether a string is acceptable as a volume id. See the
    /// [`VolumeID`] type-level doc for why the character set is what it is.
    pub fn is_valid(s: &str) -> bool {
        let len = s.len();
        if len == 0 || len > Self::MAX_LENGTH {
            return false;
    ...
    /// Get the full volume id as a string slice.
    pub fn as_str(&self) -> &str {
        &self.0
    }
    ...
    /// Get the short form (up to 8 leading characters) for display.
    ///
    /// Truncating by byte is safe because both constructors yield ASCII:
    /// [`VolumeID::parse`] admits only single-byte characters, and
    /// [`VolumeIDMint`] emits Base62. Any future `Deserialize` must parse
    /// rather than derive, or this slice can land inside a character.
    pub fn short(&self) -> &str {
        let n = self.0.len().min(Self::SHORT_LENGTH);
        &self.0[..n]
    }
...
/// Generates unique 12-character Base62 volume IDs.
///
/// Uses cryptographically random bytes mapped to the Base62 alphabet.
/// Each character encodes ~5.95 bits, yielding ~71 bits of entropy total —
/// enough that a local store never has to check for collisions before
/// creating `{home}/volumes/{id}`.
///
/// Local minting policy is independent of [`VolumeID`] validation: the
/// validator is permissive (any opaque server-issued id works), but the
/// mint always emits 12-char Base62 for volumes created locally.
pub struct VolumeIDMint;

impl VolumeIDMint {
    /// Length of locally minted volume IDs.
    pub const MINT_LENGTH: usize = 12;

    /// Mint a new unique volume ID.
    pub fn mint() -> VolumeID {
        VolumeID(mint_base62::<{ Self::MINT_LENGTH }>())
    }
}
...
mod tests {
    ...
    #[test]
    fn volume_mint_produces_a_valid_id() {
        let id = VolumeIDMint::mint();

        assert_eq!(id.as_str().len(), VolumeIDMint::MINT_LENGTH);
        assert!(
            VolumeID::is_valid(id.as_str()),
            "minted id must parse: {id}"
        );
    }

    /// A local volume's id becomes a directory name under `{home}/volumes/`,
    /// so anything that could steer that join elsewhere must not parse.
    #[test]
    fn volume_ids_that_are_not_a_single_path_component_are_rejected() {
        for bad in [
            "",
            ".",
            "..",
            ...
            assert!(VolumeID::parse(bad).is_none(), "{bad:?} must not parse");
        }
        let longest = "a".repeat(VolumeID::MAX_LENGTH);
        assert!(VolumeID::parse(&longest).is_some());
        assert!(VolumeID::parse(&format!("{longest}a")).is_none());

        assert!(VolumeID::parse("vol_abc-123").is_some());
    }
