...
impl LocalVolumeStore {
    ...
    /// server's store. No mount creates a volume: [`Self::create`], reached
    /// through `boxlite volume create`, is the only way one comes into
    /// existence. Lock-free like [`Self::get`]: it only reads, and mounting an
    /// existing volume is the common path.
...
/// `volumes/`.
///
/// `volumes/anonymous/<ulid>/` is the one that matters: the CLI still puts a
/// plain directory there for every `-v /data`, so this is current behaviour
/// and not only an upgrade leftover. Counted as a volume, `anonymous` would
/// list as one, be mountable by that name and fail at boot for want of a
/// payload, and `volume rm anonymous` would take every anonymous mount on the
/// machine with it. Excluding it costs a `warn!` per scan in
/// [`LocalVolumeStore::list`] —
/// noise this store accepts for now, and which goes away when `-v /data`
/// mints a real volume.
