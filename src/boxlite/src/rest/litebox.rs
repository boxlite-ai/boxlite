...
mod tests {
    ...
    // server hands back on the upgrade — every control op the terminal
    // needs (SIGINT on Ctrl-C, resize on SIGWINCH) is addressed by that id
    // — and stream init's output and exit code.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
