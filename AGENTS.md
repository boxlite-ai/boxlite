...
<!-- agent-tooling:guidance:begin rev=d995e4051ace sha256=9f2321203279 -->
...
- **High cohesion, loose coupling via a facade:** group related state + behavior into one type or module; expose 1–2 public entry points; keep internals and helpers private (minimizes cross-module knowledge). *Anti-pattern:* scattered free public functions callers must stitch together — leaks call order, helper graph, and shared state into every call site; every new caller re-learns the workflow. *Exception:* stateless utility modules of small pure helpers. Concrete in-repo exemplars belong in each repository's own instructions.
...
- Every PR description carries a before/after end-to-end call graph — same shape as the graph above. Bug fixes mark the faulty hop `← BUG: …` in *Before* and link the issue (`Fixes #<n>`). Enforced by the pinned agent-tooling preflight hook; rule and example in the repository's CONTRIBUTING.
