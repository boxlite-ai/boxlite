...
- `host.boxlite.internal` is a built-in hostname that resolves to
  `192.168.127.254` and reaches host loopback services. Spell it in lower
  case: the record label is compared byte for byte, so `HOST.boxlite.internal`
  answers `NXDOMAIN` rather than the host address. The record is served
  whatever `allow_net` says, but egress to that address is governed by it:
  add `"192.168.127.254"`, or a CIDR covering it, to reach it under a
  non-empty allowlist.
...
- Wildcard hostname: `"*.example.com"` (any depth of subdomain; not the apex)
