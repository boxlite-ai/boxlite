...
|--------|--------|----------------|
| `boxlite serve` | boxes, lifecycle, exec, files, snapshots, clone/export/import, metrics, config, me, volumes (its own `Volume` shape: `id`, `name`, `created_at` and a `size_bytes` the spec does not define; no `state`, `updated_at`, `last_used_at` or `error_reason`) | `network/tunnel`, the three `images/*` paths |
