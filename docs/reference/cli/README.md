...
**Synopsis:** `boxlite network tunnel BOX GUEST_PORT [--listen ADDRESS | --stdio]`

Without either option, print the public URL supplied by a remote REST service;
...
`--stdio` transfers raw bytes over stdin/stdout without printing a URL or
listener address. It is mutually exclusive with `--listen`, supports stdin EOF
as a write half-close, and exits when the remote side closes or on cancellation.

---

### `boxlite ssh`

| Command | Behavior | Default output |
|---------|----------|----------------|
...
Control status contains `enabled`, `generation` (unsigned 64-bit integer),
`listen_address`, `host_public_key`, and `host_key_fingerprint`.
Login output contains `box_id`, `login`, `port`, `identity_file`,
...
See [Guest SSH control](../../guides/ssh.md#cli-quick-start) for credential
storage, recovery, JSON configuration, and remote tunnel limitations.
