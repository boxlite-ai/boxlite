...
time — the in-progress run continues, the newest pending run waits, and intermediate
pending runs are superseded by GitHub.
...
- Push to `main` touching runtime, application, or E2E orchestration code (`src/boxlite`,
  `src/shared`, `src/cli`, `src/guest`, `apps/**`, `make/**`, `scripts/setup/**`,
  `scripts/test/e2e/**`, `sdks/**`, `Makefile`, any `Cargo.toml`, `Cargo.lock`, or the
