    ...
    try:
        ...
        while True:
            chunk = await asyncio.wait_for(
                loop.sock_recv(connection, 64 * 1024), timeout=10
            )
            if not chunk:
                return bytes(response)
            response.extend(chunk)
...
async def _websocket_echo(tunnel, marker: bytes) -> bytes:
    ...
    try:
        ...
        while b"\r\n\r\n" not in headers:
            headers.extend(
                await asyncio.wait_for(loop.sock_recv(connection, 4096), timeout=5)
            )
        ...
        await loop.sock_sendall(connection, frame)
        async def recv_exact(size: int) -> bytes:
            data = bytearray()
            while len(data) < size:
                chunk = await asyncio.wait_for(
                    loop.sock_recv(connection, size - len(data)), timeout=5
                )
                if not chunk:
...
async def test_python_sdk_tunnel_preserves_tcp_half_close(rt, image):
    async with boxlite.SimpleBox(image=image, runtime=rt, auto_remove=True) as box:
        ...
        while chunk := await asyncio.wait_for(
            loop.sock_recv(connection, 8192), timeout=5
        ):
            response.extend(chunk)
        connection.close()
