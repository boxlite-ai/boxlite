...
fn run_shim(mut config: InstanceSpec, timing: impl Fn(&str)) -> BoxliteResult<()> {
    ...
    if let Some(ref net_config) = config.network_config {
        ...
        // Inject MITM CA cert as env var for the guest agent.
        // The guest decodes it, appends to the system CA bundle, and removes
        // the env var before container processes start (container.rs).
        // This must happen here (not container_rootfs.rs) because the CA is
        // generated by gvproxy which runs in the shim subprocess.
