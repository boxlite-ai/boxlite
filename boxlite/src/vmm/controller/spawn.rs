...
/// * `config_path` - Path to serialized InstanceSpec JSON
...
pub(crate) fn spawn_subprocess(
    ...
    let shim_args = build_shim_args(engine_type, config_path);

    // Build jailer with security options and volumes
...
fn build_shim_args(engine_type: VmmKind, config_path: &Path) -> Vec<String> {
    vec![
        "--engine".to_string(),
        format!("{:?}", engine_type),
...
mod tests {
    ...
    #[test]
    fn build_shim_args_uses_config_file_flag() {
        let args = build_shim_args(VmmKind::Libkrun, Path::new("/tmp/shim-config.json"));

        assert!(
            args.windows(2)
                .any(|w| w == ["--config-file", "/tmp/shim-config.json"])
        );
