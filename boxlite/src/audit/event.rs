...
pub enum AuditEventKind {
    ...
    // ── Network (Phase 2) ───────────────────────────────────────────────
    /// Outbound network connection allowed by policy.
    NetworkConnectionAllowed { destination: String, port: u16 },

    /// Outbound network connection blocked by policy.
    NetworkConnectionBlocked { destination: String, port: u16 },

    /// DNS query allowed by policy.
    DnsQueryAllowed { hostname: String },

    /// DNS query blocked by policy.
    DnsQueryBlocked { hostname: String },

    // ── Secrets (Phase 3) ───────────────────────────────────────────────
    /// Secret value was substituted in an outbound request.
    /// The actual value is NEVER included in audit events.
    SecretSubstituted { name: String, destination: String },
}
