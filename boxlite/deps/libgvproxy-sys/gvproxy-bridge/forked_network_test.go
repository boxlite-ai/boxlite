...
// If gvisor-tap-vsock renames or removes this field, this test fails
...
func TestStackFieldExists(t *testing.T) {
	...
	if !stackField.IsValid() {
		t.Fatal("VirtualNetwork has no 'stack' field — gvisor-tap-vsock API changed. " +
			"Update OverrideTCPHandler in forked_network.go")
	}
