...
async fn test_cow_child_disks_exist_after_snapshot_create() {
    ...
    assert!(bdir.join(CONTAINER_DISK).exists(), "container disk missing before snapshot");
    assert!(bdir.join(GUEST_ROOTFS_DISK).exists(), "guest disk missing before snapshot");
    ...
    assert!(sdir.join(CONTAINER_DISK).exists(), "snapshot container disk missing");
    assert!(sdir.join(GUEST_ROOTFS_DISK).exists(), "snapshot guest disk missing");
...
async fn test_box_restartable_after_snapshot_create() {
    ...
    litebox.start().await.expect("start after snapshot create failed");
    ...
    assert_eq!(result.exit_code, 0, "box should be functional after snapshot create");
...
async fn test_box_startable_after_snapshot_restore() {
    ...
    // Box must be startable after restore.
    litebox.start().await.expect("start after restore failed");

    let cmd = BoxCommand::new("cat").args(["/root/ver.txt"]);
    let mut exec = litebox.exec(cmd).await.expect("exec failed");
    let result = exec.wait().await.expect("wait failed");
    assert_eq!(result.exit_code, 0, "cat should succeed after restore");
