...
codex_invoke_instruction="Run the Codex audit producer now:
  CODEX_COMMIT_PUSH_AUDIT_MODE=agentic bash .agents/hooks/run-commit-push-audit.sh ${kind} '<target command>'
...
if [[ -n "${CODEX_SANDBOX:-}" && -z "${GITHOOK_DELEGATED:-}" ]]; then
  ...
  if [[ "$defer_push_to_git_hook" != 1 ]]; then
    ...
    codex_auditor="$repo_root/.agents/hooks/run-commit-push-audit.sh"
