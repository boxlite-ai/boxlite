...
- Audit verdicts before ending: when you end a turn asserting something as established — a fix that works, tests that pass, a root cause, an ops/infra finding, "no issues", a factual answer — invoke the `verdict-auditor` subagent synchronously (Task, run_in_background: false) to attach proof; the auditor (never you) writes the dossier (`.claude/.last-verdict.json`). The Stop gate detects verdict-shaped final messages straight from the transcript and blocks them until a fresh dossier exists; a FAIL keeps blocking until its findings are addressed (that loop is deliberate); stale/mismatched dossiers are discarded, never blocked on; chat and question turns end freely. Detection is a curated pattern list — a creatively-worded verdict can slip it, so declaring remains your duty, not only the hook's.
