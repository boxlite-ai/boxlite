...
- Audit verdicts before ending: when you end a turn asserting something as established — a fix that works, tests that pass, a root cause, an ops/infra finding, "no issues", a factual answer — invoke the `verdict-auditor` subagent (Task) to attach proof; the auditor (never you) writes the dossier (`.claude/.last-verdict.json`). The Stop gate is delta-triggered and loop-free: a turn that changed the working tree must end with a fresh dossier (the hook blocks otherwise), while no-delta turns end freely — so audit-completion turns can't re-trigger audits and plain chat needs no audit. Verdicts that change no files (pure investigation) are still yours to declare and audit. A present-but-FAIL/stale dossier blocks until re-audited.
