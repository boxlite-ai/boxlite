...
- Audit verdicts before ending: when you end a turn asserting something as established — a fix that works, tests that pass, a root cause, an ops/infra finding, "no issues", a factual answer — invoke the `verdict-auditor` subagent synchronously (Task, run_in_background: false) to attach proof; the auditor (never you) writes the dossier (`.agents/state/last-verdict.json`). The Stop gate triages the WHOLE final turn (every assistant text since the last real user message) straight from the transcript — a fast model judges "does this state facts, findings, or outcomes that should be double-checked?", falling back to a curated pattern list (EN+中文) when no model is reachable — and blocks until a fresh dossier exists; every allow announces its decision to the human via systemMessage (invisible to the model); a FAIL keeps blocking until its findings are addressed (that loop is deliberate); stale/mismatched dossiers are discarded, never blocked on; chat and question turns end freely; a judged message is never judged twice (flush-race guard). Triage can misread — declaring remains your duty, not only the hook's.
