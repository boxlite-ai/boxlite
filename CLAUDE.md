...
- Every PR description carries a before/after end-to-end call graph — same shape as the graph above. Bug fixes mark the faulty hop `← BUG: …` in *Before* and link the issue (`Fixes #<n>`). Enforced by the pinned agent-tooling preflight hook; rule and example in [CONTRIBUTING.md](./CONTRIBUTING.md#commit--pr-messages).
