...
- Every PR description carries a before/after end-to-end call graph — same shape as the graph above. Bug fixes mark the faulty hop `← BUG: …` in *Before* and link the issue (`Fixes #<n>`). Enforced by `.agents/hooks/preflight-pr-review.sh`; rule and example in [CONTRIBUTING.md](./CONTRIBUTING.md#commit--pr-messages).
