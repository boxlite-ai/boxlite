...
- Self-declare verdicts: when you end a turn asserting something as established — a fix that works, tests that pass, a root cause, an ops/infra finding, "no issues", a factual answer — invoke the `verdict-auditor` subagent (Task) to attach proof before ending; the auditor (never you) writes the dossier (`.claude/.last-verdict.json`). You decide when a turn carries a verdict; a turn with nothing verifiable (a question, an acknowledgment, plain chat) needs no audit. The Stop gate is verdict-scoped and loop-free: a no-dossier turn is allowed, so the gate never forces an audit and a background audit's completion can't trigger another. A present-but-FAIL/stale dossier still blocks (hard) or nudges (soft) — a declared verdict that fails audit has teeth; only "no verdict" is trusted to you.
